...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('test')
    .setDescription('Test-download an Instagram reel without cookies')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const [meta, buffer] = await Promise.all([
        _dep.resolver.resolveReel(reelId, noCookies),
        _dep.resolver.downloadAsBuffer(reelId, noCookies),
      ]);
      ...
      console.error(`Test-download failed for reel ${reelId}:`, err.message);
      await interaction.editReply(
